...
def get_expected_noise(
    ...
    """
    # Get the expected noise
    erf_input = torch.tensor(2.0, dtype=torch.float64) / (
        torch.tensor(1.0, dtype=torch.float64)
        * num_pixels.to(torch.float64)
...
def calc_survival_histogram_single(
    ...
    histogram_data = torch.bincount(valid_bins, minlength=HISTOGRAM_NUM_POINTS)
...
def write_survival_histogram_single(
    ...
    with open(
        f"{output_dir}/survival_histogram_micrograph_{micrograph_number}.txt", "w"
    ) as f:
        ...
        for i in range(HISTOGRAM_NUM_POINTS):
            f.write(
                f"{temp_float + HISTOGRAM_STEP *i}, {histogram_data[i]}, {survival_histogram[i]}, {expected_survival_hist[i]}\n"
            )
...
def core_match_template_project(
    ...
    total_correlation_positions = torch.tensor(
        euler_angles.shape[0] * defocus_values.shape[0]
    )
    expected_noise = get_expected_noise(
        num_pixels=num_pixels,
        total_correlation_positions=total_correlation_positions,
    )
    survival_histogram, expected_survival_hist, temp_float, histogram_data = (
        calc_survival_histogram_single(
...
def _core_match_template_single_gpu(
    ...
    correlation_sum = torch.zeros(size=(H, W), dtype=torch.float32, device=device)
    correlation_squared_sum = torch.zeros(
        size=(H, W), dtype=torch.float32, device=device
    )
